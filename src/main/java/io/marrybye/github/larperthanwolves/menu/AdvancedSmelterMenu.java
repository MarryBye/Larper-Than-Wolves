...
public class AdvancedSmelterMenu extends AbstractContainerMenu {
    private final Container container;
    private final ContainerData data;
    private final ContainerLevelAccess levelAccess;

    // Client constructor
    public AdvancedSmelterMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf extraData) {
        this(containerId, playerInventory, new SimpleContainer(6), new SimpleContainerData(4), ContainerLevelAccess.NULL);
    }

    // Server constructor
    public AdvancedSmelterMenu(int containerId, Inventory playerInventory, Container container, ContainerData data, ContainerLevelAccess levelAccess) {
        super(ModMenuTypes.ADVANCED_SMELTER.get(), containerId);
        ...
        this.addSlot(new SmelterInputSlot(container, 0, 56, 17));
        this.addSlot(new SmelterInputSlot(container, 1, 56, 35));
        this.addSlot(new SmelterInputSlot(container, 2, 56, 53));
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            // From player inventory / hotbar (6..41)
                ...
                if (!FoodCookingRegistry.isFood(stackInSlot)) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(this.levelAccess, player, ModBlocks.ADVANCED_SMELTER.get());
    }
    ...
    public static class SmelterInputSlot extends Slot {
        public SmelterInputSlot(Container container, int slot, int x, int y) {

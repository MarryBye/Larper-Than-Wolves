...
public class FuelRegistry {
    ...
    public static FuelInfo getFuelInfo(ItemStack stack) {
        ...
        // 8. Blaze Rod (Tier 8: Nether heat rod - 270s / 4.5 min, 70t cook)
        if (stack.is(Items.BLAZE_ROD)) {
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.blazeRodBurnTicks.get() : 5400;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.blazeRodCookSpeed.get() : 70;
            return new FuelInfo(burn, speed);
        }

        // 7. Coal (Tier 7: Mineral coal - 190s / 3m 10s, 100t cook)
        if (stack.is(Items.COAL)) {
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.coalBurnTicks.get() : 3800;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.coalCookSpeed.get() : 100;
            return new FuelInfo(burn, speed);
        }

        // 6. Charcoal (Tier 6: Wood charcoal - 160s / 2m 40s, 120t cook)
        if (stack.is(Items.CHARCOAL)) {
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.charcoalBurnTicks.get() : 3200;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.charcoalCookSpeed.get() : 120;
            return new FuelInfo(burn, speed);
        }

        // 5. Logs, Wood, Stripped Logs & Tree Stumps (Tier 5: Dense timber - 110s / 1m 50s, 150t cook)
        if (stack.is(ItemTags.LOGS) || Block.byItem(stack.getItem()) instanceof StumpBlock) {
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.logBurnTicks.get() : 2200;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.logCookSpeed.get() : 150;
            return new FuelInfo(burn, speed);
        }

        // 4. Planks & Wooden Construction Blocks (Tier 4: Processed lumber - 70s / 1m 10s, 180t cook)
        if (stack.is(ItemTags.PLANKS) || stack.is(ItemTags.WOODEN_DOORS) || stack.is(ItemTags.WOODEN_PRESSURE_PLATES)
            ...
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.plankBurnTicks.get() : 1400;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.plankCookSpeed.get() : 180;
            return new FuelInfo(burn, speed);
        }

        // 3. Wooden Slabs, Stairs, Fences, Gates & Trapdoors (Tier 3: Small wooden parts - 50s, 200t cook)
        if (stack.is(ItemTags.WOODEN_SLABS) || stack.is(ItemTags.WOODEN_STAIRS) || stack.is(ItemTags.WOODEN_TRAPDOORS)
            ...
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.woodenSlabBurnTicks.get() : 1000;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.woodenSlabCookSpeed.get() : 200;
            return new FuelInfo(burn, speed);
        }

        // 2. Sticks, Pointed Sticks & Small Wood (Tier 2: Sticks - 35s, 240t cook)
        if (stack.is(Items.STICK) || stack.is(ModItems.POINTED_STICK.get()) || stack.is(Items.BOWL)) {
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.stickBurnTicks.get() : 700;
            int speed = ModConfig.SERVER != null ? ModConfig.SERVER.stickCookSpeed.get() : 240;
            return new FuelInfo(burn, speed);
        }

        // 1. Foliage, Twigs, Dry Grass, Saplings & Leaves (Tier 1: Kindling - 25s, 260t cook)
        if (stack.is(ModItems.TWIG.get()) || stack.is(ModItems.DRY_GRASS.get()) || stack.is(Items.DEAD_BUSH)
            ...
            int burn = ModConfig.SERVER != null ? ModConfig.SERVER.foliageBurnTicks.get() : 500;

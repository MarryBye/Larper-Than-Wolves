# 🐺 Larper Than Wolves - Полная сводка проекта (v1.8.0)
...
* **v1.7.0**:
  * Крафты кремниевых инструментов (топор, кирка, лопата, копьё, ножницы) переведены на полноценный кремень (`minecraft:flint`) вместо осколков кремния.
  * Устранено автоматическое самовоспламенение кирпичной печи и смешивателя сплавов — теперь они зажигаются строго вручную игроком с помощью зажигалки / огнива.
  * Полное и гарантированное удаление дропа отключённых предметов с мобов (очистка экипировки, отмена спавна ItemEntity, очистка `LivingDeathEvent` / `LivingDropsEvent`).
  * Из гравия теперь выпадают только осколки кремния, медная пыль или сам гравий (обычный кремень больше никогда не выпадает из гравия).
  * Исправлена совместимость `TrunkPlacerMixin` с модами оптимизации мира (C2ME, MixinExtras `@WrapOperation`).
* **v1.8.0** (Архитектурный рефакторинг, универсальность и исправление багов):
  * **AGENTS.md**: создан подробный гайд для ИИ-агентов по кодовой базе, архитектуре, паттернам добавления предметов/блоков/тиров и частым ошибкам.
  * **Рефакторинг системы сплавов (`AlloyRecipe`, `AlloyRegistry`)**: смешиватель сплавов полностью переведён на модульную дата-ориентированную систему рецептов. Добавление новых сплавов теперь происходит в 1 строчку, а JEI и фильтры воронок синхронизируются автоматически.
  * **Рефакторинг системы плавки (`SmeltingRegistry`)**: централизована логика кастомного обжига руд в самородки и приготовления пищи.
  * **Рефакторинг системы топлива (`FuelRegistry`)**: устранено дублирование кода и циклическая зависимость между Кирпичной печью и Смешивателем сплавов.
  * **Исправление критического бага пропажи предметов (`onRemove`)**: Кирпичная печь и Смешиватель сплавов теперь корректно выбрасывают всё своё содержимое на землю при их разрушении.
  * **Исправление бага слота второй руки (`EquipmentSlot`)**: устранена ошибка списания прочности основного слота при использовании зажигалки/огнива/стамески во второй руке.
  * **Исправление генератора данных рецептов (`RecipeProvider`)**: исправлен `HolderLookup.Provider` в датагене, сборка `runData` теперь проходит безупречно.
  * **Оптимизация производительности**: устранено создание лишних массивов слотов в `SieveBlockEntity` при опросе воронками; заменён `Random` на потокобезопасный `ThreadLocalRandom` в `BlockBreakHandler`.
  * **Исправление дропа блоков не-каменного типа**: ломание земли/дерева кремниевыми и медными кирками больше не удаляет их дроп.
